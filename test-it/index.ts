...
describe('DECP processing', () => {
  ...
  //   const mockData = (path.join(import.meta.dirname, 'resources/extrait-decp-global.json'))

  //   const { writeFlattenData } = await import('../lib/convert.ts')
  //   const resPath = await writeFlattenData(mockData, mappingMarche, 'marches.marche', pathDir, 'extrait-decp-global_flatten.json', log)
  ...
  // })
  // it('initialize : upload all year decp (no mock)', async function () {
  //   const context = testUtils.context({
  //     processingConfig: {
  //       datasetMode: 'create',
  //       dataset: { title: 'decp' },
  ...
  it('initialize : upload all year decp (no mock)', async function () {
    ...
    const { initialize } = await import('../lib/execute.ts')
    const url = 'https://www.data.gouv.fr/api/1/datasets/r/bd33e98f-f8e3-49ba-9f26-51c95fe57234'
    const filters = ['marches.marche', 'marches.contrat-concession']
    await initialize(mapping, filters, url, context)
  })
